...
export class MessageComponent implements OnInit{
    ...
    this.user = new User("","","","","","","",false,0,0,"","","","",360);
    ...
    ngOnInit(){
      ...
      friendsSocket(){
        ...
        this.socket.on("friendWriting", (user) =>{
          if(user && this.friend && this.friend._id == user) this.userWriting = true;
        })
        this.socket.on("stopWriting", (user) =>{
          if(user && this.friend && this.friend._id == user) this.userWriting = false;
        })
        this.socket.on("newMessage", (message, emitter, created_at) =>{
          if(this.openMessageBox && this.friend._id == emitter._id){
            let newMessage = {  text : message.text, viewed: "false", emitter, created_at : created_at }
            this.messagesArr.push(newMessage)
            this.setViewedMessages(this.friend); 
            setTimeout(()  =>{
              if(this.chatbox.nativeElement.scrollTop > this.chatbox.nativeElement.scrollHeight - this.reduce ){
        ...
        this.socket.on("friendViewedMessages", (user) =>{
          if(this.openMessageBox && this.friend && this.friend._id == user._id) this.reciveMessages();
        })
    }
      emitWriting(e){
    ...
    onSubmit(form){
      ...
      this._messageService.sendMessage(this.token, this.message).subscribe(
			response => {
        let newMessage = {text: this.message.text ,viewed: "false" ,emitter: this.identity, created_at :response.message.created_at}
        this.messagesArr.push(newMessage)
    ...
    this._messageService.userMessages(this.token, this.friend._id).subscribe(
      response => {
        ...
        setTimeout(()  =>{
          this.chatbox.nativeElement.scrollTop = this.chatbox.nativeElement.scrollHeight  
          this.reduce = this.chatbox.nativeElement.scrollHeight - this.chatbox.nativeElement.scrollTop + 100;
        },100)
      },
      error => {
        console.log(<any>error);
    ...
    friendMessages(messages, friends){
      ...
      else if(messages == "none" && friends == "block" && this.writing == true){
        this.socket.emit("noWriting", this.identity._id, this.friend)
        this.writing = false;
        this.openMessageBox = false;
      } 
      else if(messages == "none" && friends == "block") this.openMessageBox = false;
    }
    ...
    doDeleteFriend(userToDelete){
      ...
      else if(userToDelete.followed._id) this.userToRecive = userToDelete.followed._id;
      this.userToDelete_id = userToDelete._id;
      if(userToDelete.followed.nick) this.userToDelete_nick = userToDelete.followed.nick;
      else if(userToDelete.user.nick) this.userToDelete_nick = userToDelete.user.nick;
    }

    deleteFriend(){
      this._followService.deleteFollow(this.token, this.userToDelete_id).subscribe(
        response => {
          this.getFollows();
