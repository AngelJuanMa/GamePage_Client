...
export class ChatboxComponent implements OnInit {
  ...
  constructor(
    ...
    this.sala = new Sala("","",null,"","","","","",null,"","",false,false,false,false);
    ...
    userSocket(){
      ...
      messageSocket(){
        ...
        this.messageIo.on("chatbox todos:input", (messageTodos) => {
          ...
          setTimeout(()  =>{
              if(this.chatboxTodos.nativeElement.scrollTop >= this.chatboxTodos.nativeElement.scrollHeight - 190)
          ...
          this.messageIo.on("chatbox pride:input", (messagePride) => {
            ...
            setTimeout(()  =>{
              if(this.chatboxPride.nativeElement.scrollTop >= this.chatboxPride.nativeElement.scrollHeight - 190)
      ...
      sendMessage(form){
        if(this.message.text){
          if(!this.pride){
            ...
            this.messageIo.emit("chatbox pride:send", this.message , this.identity.pride)
            setTimeout(()  =>{
              this.chatboxPride.nativeElement.scrollTop = this.chatboxPride.nativeElement.scrollHeight 
              this.message.text = null;
            }, 100)
        ...
        todosHover(bool){
          if(bool && !this.pride) this.todos.nativeElement.style.background = "rgb(150, 148, 148)";
          else if(bool) this.todos.nativeElement.style.background = "rgb(61, 62, 60)";
          else if(!bool && !this.pride) this.todos.nativeElement.style.background = "rgb(132, 130, 130)";
          else if (!bool && this.pride) this.todos.nativeElement.style.background = "#1d1e1a"; 
        }

        clanHover(bool){
          if(bool && this.pride) this.clan.nativeElement.style.background = "rgb(150, 148, 148)";
          else if(bool) this.clan.nativeElement.style.background = "rgb(61, 62, 60)";
          else if(!bool && this.pride) this.clan.nativeElement.style.background = "rgb(132, 130, 130)";
          else if (!bool && !this.pride) this.clan.nativeElement.style.background = "#1d1e1a"; 
